...
<li><strong data="opt_timerSize">Величина мерача</strong>. Set the size of main timer.</li>
<li><strong data="opt_smallADP">користити мали фонт након децималног зареза</strong>. Use a smaller font size after the digital point in main timer.</li>
<li><strong data="opt_color">Изаберите боју теме</strong>. Select color schemes of csTimer. Click csTimer's logo to show more color schemes.</li>
<li><strong data="opt_useMouse">use mouse timer</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
<li><strong data="opt_useIns">користите WCA инспекције</strong>. Enable WCA inspection procedure, which is a 15-second countdown, auto +2/DNF penalty will also be enabled if you inspecting more than 15 seconds.</li>
<li><strong data="opt_voiceIns">глас упозорење од WCA инспекције</strong>. Alert at 8s/12s of inspection, to simulate the alert from judge in WCA competitions.</li>
...
<li><strong data="opt_phases">више фаза</strong>. Number of phases, press any key to mark a split point when timing.</li>
...
<li><strong data="opt_scrSize">Величина скрембла</strong>. Size of the scramble text.</li>
...
<li><strong data="opt_scrMono">Скрембл са једноструким размацима</strong>. Use monospaced font for scramble text.</li>
<li><strong data="opt_scrLim">Ограничите висину простора за скрембл</strong>. When the scramble area is too high, a scroll bar will occur to avoid the raising of the scramble panel.</li>
<li><strong data="opt_scrAlign">Поравнавање скрембл простора</strong>. Alignment of the whole scramble area, include scramble type selector.</li>
...
<li><strong data="opt_scrFast">Користи брзи скрембл за 4 x 4 x 4 (незванична)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
<li><strong data="opt_scrKeyM">Означи кључни потез(е) у скремблу</strong>. Mark a key move in the scramble, e.g. the move that take the state away from square shape in SQ1 scrambles.</li>
...
<li><strong data="opt_useKSC">Пречице за тастатуру</strong>. Use keyboard shortcut to switch scramble type, generate next scramble, switch between sessions. Click csTimer's logo to show details.</li>
...
<li><strong data="tool_distribution">Дистрибуција времена</strong>. Time distribution and stability analysis, &lt;X Y/Z means there are totally Z solves less than X seconds, and all of the latest Y solves are less than X seconds in the session.</li>
