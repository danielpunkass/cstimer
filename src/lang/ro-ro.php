...
<ul>
<li><strong data="opt_ahide">Ascunde elementele in timpul cronometrării</strong>. Hide logo and all panels when timing.</li>
<li><strong data="opt_useMilli">utilizeaza milisecunde</strong>. Display the millisecond digit, no matter whether it is checked, the internal timing accuracy of csTimer is 1 millisecond.</li>
<li><strong data="opt_timeFormat">Formatul timpului</strong>. Time format to display.</li>
...
<li><strong data="opt_zoom">Marire</strong>. You can adjust sizes of all elements by this option.</li>
<li><strong data="opt_font">Alegeți fontul cronometrului</strong>. Font of the main timer.</li>
...
<li><strong data="opt_view">Stilul UI-ului este</strong>. Switch between desktop and mobile views.</li>
<li><strong data="opt_wndScr">Stil afișare panou de amestecare</strong>. Make scramble panel embedded into background.</li>
<li><strong data="opt_wndStat">Stil afișare panou de statistici</strong>. Make list times panel embedded into background.</li>
<li><strong data="opt_wndTool">Stil afișare panou de unelte</strong>. Make tool panel embedded into background.</li>
...
<li><strong data="opt_timerSize">marimea timer-urui</strong>. Set the size of main timer.</li>
<li><strong data="opt_smallADP">A se utiliza font mic după zecimală</strong>. Use a smaller font size after the digital point in main timer.</li>
<li><strong data="opt_color">Alegeți culoarea temei</strong>. Select color schemes of csTimer. Click csTimer's logo to show more color schemes.</li>
<li><strong data="opt_useMouse">folosește cronometrul mouse-ului</strong>. Use mouse to start timer, keyboard-trigger will also be available.</li>
<li><strong data="opt_useIns">utilizaţi inspectie WCA</strong>. Enable WCA inspection procedure, which is a 15-second countdown, auto +2/DNF penalty will also be enabled if you inspecting more than 15 seconds.</li>
<li><strong data="opt_voiceIns">alerta vocala de la inspectie WCA</strong>. Alert at 8s/12s of inspection, to simulate the alert from judge in WCA competitions.</li>
<li><strong data="opt_voiceVol">Voice volume</strong>. Voice level of the alert above.</li>
<li><strong data="opt_input">intrând în timpuri cu</strong>. csTimer is able to add solves by several ways, it supports manually input, automatically record from a stackmat timer, connect to a bluetooth smart cube or play virtual Rubik's cube, besides keyboard timing.</li>
...
<li><strong data="opt_timeU">actualizarea temporizatorului este</strong>. How timer is updated when timing.</li>
<li><strong data="opt_preTime">timpul de menținere a spațiului în minus (secunde))</strong>. How long the space bar should be held before the timer turns green.</li>
<li><strong data="opt_phases">mai multe faze</strong>. Number of phases, press any key to mark a split point when timing.</li>
...
<li><strong data="opt_scrSize">Mărimea amestecării.</strong>. Size of the scramble text.</li>
...
<li><strong data="opt_scrMono">amestecare monospațiată</strong>. Use monospaced font for scramble text.</li>
<li><strong data="opt_scrLim">Limitează înălțimea zonei de amestecare</strong>. When the scramble area is too high, a scroll bar will occur to avoid the raising of the scramble panel.</li>
<li><strong data="opt_scrAlign">Alinierea zonei amestecării</strong>. Alignment of the whole scramble area, include scramble type selector.</li>
...
<li><strong data="opt_scrFast">Amestecare rapida pentru 4x4x4 (non-oficială)</strong>. WCA official 4x4x4 scramble requires huge computation resources, select this option to use a random-move scramble for 4x4x4 instead.</li>
<li><strong data="opt_scrKeyM">Etichetați mișcarea (mutările) cheii în amestecare</strong>. Mark a key move in the scramble, e.g. the move that take the state away from square shape in SQ1 scrambles.</li>
...
<li><strong data="opt_imgSize">Mărimea imaginii amestecării</strong>. Set the size of scramble image.</li>
<li><strong data="opt_NTools">Număr de unelte</strong>. csTimer is able to show up to 4 tools simultaneously.</li>
<li><strong data="opt_useKSC">Folosește scurtătura tastaturii</strong>. Use keyboard shortcut to switch scramble type, generate next scramble, switch between sessions. Click csTimer's logo to show details.</li>
...
<li><strong data="opt_vrcSpeed">Viteza de bază VRC (tps)</strong>. Base turn speed of the virtual Rubik's cube, the turn will be speed up if there are multiple moves to turn.</li>
<li><strong data="opt_vrcMP">Multi-fază</strong>. Automatic multi-phase split for virtual Rubik's cube and bluetooth cube.</li>
...
<li><strong data="tool_cfm">Confirmă timpul</strong>. Tool to view current solves with its comment, scramble, solving date and reconstruction if available, which is also the dialog when you click on a solve.</li>
<li><strong data="tool_hugestats">statisticile sesiunilor</strong>. You are able to do cross-session statistics with this tool.</li>
<li><strong data="tool_stats">Statistici</strong>. Statistic table similar with the table in the list times panel.</li>
<li><strong data="tool_distribution">distribuția timpului</strong>. Time distribution and stability analysis, &lt;X Y/Z means there are totally Z solves less than X seconds, and all of the latest Y solves are less than X seconds in the session.</li>
<li><strong data="tool_trend">trend timp</strong>. Shows a trend curve of all solves in current session.</li>
<li><strong data="tool_dlystat">Daily Statistics</strong>. Count number of solves each day/week/month/year.</li>
<li><strong data="tool_image">desenează amestec</strong>. Scramble image to verify a correct scramble, all WCA puzzles are supported.</li>
...
<li><strong data="tool_cross">Solvers &gt; cruce</strong>. Cross solver, which solve DF, DL, DR, DB edges.</li>
...
<li><strong data="tool_giikerutil">Cub Bluetooth</strong>. Auxiliary tool for bluetooth cube, which is able to show current state, battery power, real-time reconstruction etc.</li>
